...
export default function SpecularCard({
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      if (prefersReduced) {
        renderer.render({ scene: mesh }) // reduced-motion：只渲染一帧静态，不循环
      } else {
        raf = requestAnimationFrame(update)
      }
      } catch (err) {
        console.warn('[SpecularCard] WebGL unavailable, card effect disabled:', err)
        running = false
        try { if (gl && gl.canvas && gl.canvas.parentNode === fx) fx.removeChild(gl.canvas) } catch (e2) {}
        try { if (gl) gl.getExtension('WEBGL_lose_context')?.loseContext() } catch (e2) {}
